...
tap.test('testSocketIO', function (test) {
  ...
  setTimeout(function () {
    ...
    var msgStart = { name: 'hithere', body: { sourcedest: 'PAR',
        statement: query,
        op: 'START',
        settings: {
          continuous: true,
    ...
    var msgChange = { name: 'hithere', body: { sourcedest: 'PAR',
        statement: query,
        op: 'CHANGE',
        settings: {
          continuous: true,
    ...
    conversationid: 'conv1' }
    var msgStop = { name: 'hithere', body: { sourcedest: 'PAR',
        statement: query,
        op: 'STOP',
        settings: {
          continuous: false,
    ...
    sender.emit('sqlclient', { name: 'hithere', body: { sourcedest: 'PAR',
        statement: query,
        op: 'START',
        settings: {
          continuous: true,
    ...
    sender.emit('sqlclient', { name: 'hithere', body: { sourcedest: 'EXEC',
        statement: query
      },
    conversationid: 'conv2', id: 'conv2', conversationID: 'conv2' }
    )
    console.log('emitted')
    var cnt = 0
  /*
